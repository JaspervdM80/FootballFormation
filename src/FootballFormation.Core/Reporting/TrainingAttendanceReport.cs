...
public class PlayerTrainingAttendance
{
    ...
    /// Sessions that went ahead while she was a full member of that season's squad — never every session in the season, or a player who
    /// joined in January reads as having missed the autumn.
...
{
    /// Sessions that took place. <see cref="Cancelled"/> ones are on file so the week reads honestly, and are nobody's absence.
    public int Held { get; init; }
...
{
    public static TrainingAttendance Build(IReadOnlyList<Training> trainings, SeasonSquads squads)
    {
        var held = trainings.Where(t => !t.DidNotTakePlace).ToList();
        ...
        {
            Held = held.Count,
            Cancelled = trainings.Count - held.Count,
            Players =
            [
                .. expected
                    .Select(p => BuildFor(p, held, squads))
    ...
    public static PlayerTrainingAttendance BuildFor(
        Player player, IEnumerable<Training> trainings, SeasonSquads squads)
    {
        var held = 0;
        var attended = 0;
        ...
        {
            // A cancelled evening is nobody's absence, and a guest was never expected at one.
            if (training.DidNotTakePlace || !squads.For(training.SeasonId).IsFullMember(player.Id)) continue;
