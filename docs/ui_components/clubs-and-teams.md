...
resolving. Because it is a relative path, swapping a crest is a file drop — and `TeamService`
enforces the path rather than trusting the helper text, refusing an absolute URL, a protocol-relative `//host/…`
