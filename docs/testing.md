...
| Games and comments | `GameServiceTests`, `GameCommentTests` | Season derivation on create, scalar-only update, the public/private split, and that a single game comes back with its whole graph — `GetByIdAsync` and `GetLiveAsync` compose the same `GameQueries` shapes, and a level dropped from one of them fails silently in the app but fails a test here |
