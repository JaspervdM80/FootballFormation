...
CI runs `dotnet build -c Release` and `dotnet test` as a **gate on the merge**: it is one of the
three checks that have to be green before a pull request can land, and since landing is what deploys, a
