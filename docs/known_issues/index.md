...
- [EF Core](ef-core.md) — UNIQUE constraints on save, value converters, the DateInSqlInterceptor, migration ordering and the folded `InitialCreate`, connection pooling holding a file handle open, cross-context transactions.
...
- [CSS scoping](css-scoping.md) — a class with no owning `.razor.css` silently does nothing, including on a MudBlazor component's root; a name shared with an `app.css` rule; `color-mix` breaking the html2canvas export.
...
- [General](general.md) — the published-app working-directory trap, `.count()` failing open in Playwright, dirty state after a CI retry, path separators in the coverage report on Windows, the CRLF the Windows clipboard hands back, and the September-only disagreement between JavaScript's `en-GB` and .NET's `en`.
