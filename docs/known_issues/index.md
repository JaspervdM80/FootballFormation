...
- [General](general.md) — the published-app working-directory trap, `.count()` failing open in Playwright, dirty state after a CI retry, path separators in the coverage report on Windows, the CRLF the Windows clipboard hands back, `color-mix` breaking the html2canvas export, the September-only disagreement between JavaScript's `en-GB` and .NET's `en`, how slowly a subscribed calendar picks up a change, coverage not seeing untracked files, and `visual-check.sh` driving a dev server already on its port.
