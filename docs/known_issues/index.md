...
- [EF Core](ef-core.md) — UNIQUE constraints on save, value converters, the DateInSqlInterceptor, migration ordering and the folded `InitialCreate`, cross-context transactions.
...
- [Touch / PWA](touch-pwa.md) — touch-target sizing, the date picker on a phone, dialog sheets, drag-and-drop shims, and circuit reconnection after backgrounding.
- [Localization](localization.md) — resource-key homographs and the case-insensitivity trap in `ServiceOperation` action phrases.
- [Blazor components](blazor-components.md) — `section` as a reserved word, the SDK-dependent build gap, `@inherits` in the `.razor` file, generic dialog results.
...
- [General](general.md) — the published-app working-directory trap, `.count()` failing open in Playwright, and dirty state after a CI retry.
